...
def LSExplicit_CUR(X, Y, f_X, prev, beta_init, W, theta):
    i = 0
    beta = beta_init
    Xplus = X + beta * (Y - X)
    [f_t, _] = fun_grad_f_CUR(W, Xplus)
...
def IPG_ELS(W, X0, f_X, gradf_X, gamma_1, gamma_2, theta, tau, alpha, t_k, kmax, prec, lambda_row, lambda_col, FOpt=None, mu=0.01):
    ...
    while k <= kmax:
        ...
        X = Xplus
        f_X = fun_Xplus
        [_, gradf_X] = fun_grad_f_CUR(W, X)
        k += 1
        end_time = time.time()
        
        F_func.append(fun_F_CUR(W, Xplus, lambda_row, lambda_col, mu))
...
def IPG_Fixstep(W, X0, f_X, gradf_X, sigma, t_k, gamma, kmax, prec, lambda_row, lambda_col, FOpt, mu=0.01):
    ...
    while k <= kmax:
        ...
        [f_X, gradf_X] = fun_grad_f_CUR(W, X)
        k += 1
        end_time = time.time()
        
        F_func.append(fun_F_CUR(W, X, lambda_row, lambda_col, mu))
        X_func.append(np.copy(X)) 
